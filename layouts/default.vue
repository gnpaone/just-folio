<template>
  <main :class="{ 'no-touch': !isTouch, device: isDevice, mobile: isPhone }">
    <div
      class="loader"
      ref="loader"
      :style="{ 'background-color': project ? project.color : '#000000' }"
    ></div>
    <no-ssr>
      <v-turn v-if="isDevice"></v-turn>
    </no-ssr>
    <v-link route="index" className="index" delay="100"
      ><span class="label">Kiran Kumar M</span></v-link
    >
    <v-link route="projects" className="projects" delay="600"
      ><span class="label">Project</span></v-link
    >
    <v-link route="contact" className="contact" delay="1100"
      ><span class="label">Contact</span></v-link
    >
    <v-link
      route="projects"
      className="close"
      delay="0"
      v-if="$route.name === 'project-slug'"
      ><span class="glyph"></span><span class="label">Close</span></v-link
    >
    <div class="white-layer" v-if="$route.name === 'project-slug'"></div>
    <no-ssr>
      <v-scene ref="scene" v-if="isReady && !isPhone"></v-scene>
      <v-scene
        ref="scene"
        v-else-if="
          isReady &&
            isPhone &&
            $route.name !== 'projects' &&
            $route.name !== 'project-slug'
        "
      ></v-scene>
      <v-scene
        ref="scene"
        v-else-if="isReady && isPhone && $route.name === 'projects'"
        :style="{ display: 'none' }"
      ></v-scene>
      <v-scene
        ref="scene"
        v-else-if="isReady && isPhone && $route.name === 'project-slug'"
        :style="{ display: 'none' }"
      ></v-scene>
    </no-ssr>
...
export default {
  ...
  computed: {
    ...mapState(["route"]),
    ...mapGetters(["isDevice", "isPhone", "project"])
  },
  components: {
    vLink,
    vScene: () => import("~/components/common/scene.vue"),
    vTurn
  },
  methods: {
    resize() {
      ...
      if (this.$refs.page && this.$refs.page.$children[0])
        this.$refs.page.$children[0].resize &&
          this.$refs.page.$children[0].resize(w, h, pageHeight);
    ...
    tick() {
      ...
      if (this.$refs.page.$children[0])
        this.$refs.page.$children[0].tick &&
          this.$refs.page.$children[0].tick(scrollTop, realScrollTop);
    ...
    setRouterHooks() {
      this.$router.beforeEach((to, from, next) => {
        if (this.$refs.scene)
          this.$refs.scene.beforeChangePage(from.name, to.name);
        this.pageFadeOut(next);
        if (
          from.name === "project-slug" &&
          to.name === "projects" &&
          !this.isDevice
        )
      ...
      this.$router.afterEach((to, from) => {
        ...
        this.resize();
        if (to.name !== "projects") setTimeout(this.resize.bind(this), 200);
        if (this.isTouch) {
          this.animation = anime({
            targets: this.$refs.scroll,
            opacity: 1,
